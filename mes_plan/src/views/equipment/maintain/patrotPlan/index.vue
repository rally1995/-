    ...
    ebrain-main.patroplan
        ...
        ebrain-dialog(
            ...
            template(#stytype="{value}")
                el-col(:span="12")
                     el-form-item(label-width="110px" label="执行类型" :required="true" prop="exeType")
            ...
            template(#way)
                el-col(:span="12")
                     el-form-item(label-width="110px" label="循环方式" :required="true" prop="isSingle")
        ...
        ebrain-dialog(
            ...
            template(#patrol="{value}")
                    el-col(:span="12")
                        el-form-item(label-width="110px" label="点巡检方案" :required="true" prop="pointCheckPlanName")
                            el-input(style="cursor:pointer" :value="value" :readonly="true")
                                el-button(slot="append" :disabled="isPartol"  @click="dialogSearchPatrol={};openPatrol()"  icon="iconfont iconsearch")
        ebrain-dialog-table(
            height="50vh"
            :data="tableExecutor"
        ...
        ebrain-dialog-export(
            ...
            accept="xls,xlsx"
            :export-url="'/devicemanager/pointcheckschedulede/exportList/'+this.pointCheckScheduleNo"
            :params="exportIds"
...
export default {
    ...
    data() {
        return {
            pointCheckScheduleNo: '',
            btnsTop: [{
                label: '新增',
                click: () => this.newRow(),
                className: 'iconfont iconstatus-Preserve-and-add',
                permi: 'devicemanager_pointcheckschedule_add',
            }, {
                label: '修改',
                click: () => this.editRow(),
                className: 'iconfont iconstatus-edit',
                disabled: () => this.editDisabled,
                permi: 'devicemanager_pointcheckschedule_edit',
            }, {
                label: '删除',
                click: () => this.delRow(),
                className: 'iconfont iconstatus-delete',
                disabled: () => this.delDisabled,
                permi: 'devicemanager_pointcheckschedule_dels',
            }, {
                label: '启用',
                click: () => this.start(),
                className: 'iconfont iconstatus-start-up',
                disabled: () => this.startDisabled,
                permi: 'devicemanager_pointcheckschedule_start',
            }, {
                label: '停用',
                click: () => this.stop(),
                className: 'iconfont iconState-Stop',
                disabled: () => this.stopDisabled,
                permi: 'devicemanager_pointcheckschedule_stop',
            }, {
                label: '复制',
                click: () => this.copy(),
                className: 'iconfont iconstatus-copy',
                disabled: () => this.editDisabled,
                permi: 'devicemanager_pointcheckschedule_copy',
            }, {
                label: '搜索',
            ...
            btnsDown: [{
                ...
                className: 'iconfont iconstatus-Preserve-and-add',
                permi: 'devicemanager_pointcheckschedulede_add',
            }, {
                label: '删除',
                click: () => this.delRowSon(),
                className: 'iconfont iconstatus-delete',
                disabled: () => this.delDisabledSon,
                permi: 'devicemanager_pointcheckschedulede_dels',
            }, {
                label: '导出',
                click: () => this.exportAll(),
                className: 'iconfont iconstatus-Export_all',
                disabled: () => this.exportDisabled,
                permi: 'devicemanager_pointcheckschedulede_export',
            }],
            dialogForm: [{
                label: '计划编号',
                prop: 'pointCheckScheduleNo',
    ...
    computed: {
        ...
        tableColumnExecutor() {
            const row = this.dialogForm.find(v => v.prop === 'exeType');
            let arr = [];
            if (row.value === '0') {
                arr = [{
                    label: '员工姓名',
    ...
    watch: {
        ...
        underpageTotal(val) {
            const tr = this.dialogForm.find(v => v.prop === 'equipNum');
            tr.value = val;
        },
        dialogVisible(val) {
            if (!val) {
                ...
                this.selecSon = [];
                this.$refs.tableDialog.clearSelection();
            } else {
                this.undercurrentPage = 1;
                this.underpageSize = 20;
        ...
        dialogVisibleSon(val) {
            if (!val) this.isPartol = true;
        },
        dialogVisabledExport(val) {
            if (val) {
                this.exportFilename = '点巡检设备清单明细';
            }
        },
        dialogType(val) {
            if (val === 'edit') {
    ...
    mounted() {
        Promise.all([this.queryDict('PERIODIC_TYPE'),
            ...
            this.queryDict(`patrolInspection`),
        ]).then(() => {
            this.getSearchData();
        });
    },
    methods: {
        ...
        getSearchData() {
            ...
            return this.$http.get(`/devicemanager/pointcheckschedule/page`, {
                params
            }).then(({ code, data }) => {
        ...
        newRow() {
            ...
            this.dialogForm.forEach(v => {
                if (v.prop === 'pointCheckScheduleNo' || v.prop === 'scheduleStatus' || v.prop === 'equipNum'
                || v.prop === 'createBy' || v.prop === 'createTime' || v.prop === 'updateBy' ||
                v.prop === 'updateTime' || v.prop === 'nextTime') { v.disabled = true }
        ...
        editRow(row) {
            ...
            this.changeVal = this.parentRow.isSingle;
            this.palnStatus = this.parentRow.scheduleStatus;
            this.dialogForm.forEach(v => {
                if (v.prop === 'exe') v.value = this.parentRow[exeTypeType[this.parentRow['exeType']]];
                else v.value = this.parentRow[v.prop];
            });
            if (this.parentRow['scheduleStatus'] !== '0') {
                this.isEdit = true;
                this.dialogForm.forEach(v => {
                    // if (v.prop === 'pointCheckScheduleNo' || v.prop === 'scheduleStatus' || v.prop === 'equipNum'
                    // || v.prop === 'createBy' || v.prop === 'createTime' || v.prop === 'nextTime'
                    // || v.prop === 'updateBy' || v.prop === 'updateTime') { v.disabled = true }
                ...
                this.dialogForm.forEach(v => {
                    if (v.prop === 'pointCheckScheduleNo' || v.prop === 'scheduleStatus' || v.prop === 'equipNum'
                    || v.prop === 'createBy' || v.prop === 'createTime' || v.prop === 'updateBy'
                    || v.prop === 'updateTime' || v.prop === 'nextTime') {
                        v.disabled = true;
                    } else if (v.prop === 'pointCheckRate' || v.prop === 'cycleNum') {
                        if (this.changeVal === '1') {
                            v.disabled = false;
                        } else {
                            v.disabled = true;
        ...
        delRow() {
            ...
            const palnCode = this.selection.map(v => v.pointCheckScheduleNo);
            this.$confirm('是否确认删除?', '提示', {
                type: 'warning'
            }).then(() => {
                this.submitLoading = true;
                this.$http.delete('/devicemanager/pointcheckschedule/removeByIds', {
                    data: ids,
                }).then(() => {
        ...
        start() {
            const startUp = () => {
                this.$http.put(`/devicemanager/pointcheckschedule/startById`, null, {
                    params: {
            ...
            if ((new Date()).getTime() > (new Date(this.parentRow['startTime'])).getTime()) {
        ...
        stop() {
            this.$http.put(`/devicemanager/pointcheckschedule/stopById`, null, {
                params: {
                    id: this.parentRow['id']
                }
            }).then(() => {
        ...
        copy() {
            this.$http.put(`/devicemanager/pointcheckschedule/copyById`, null, {
                params: {
                    id: this.parentRow['id']
                }
            }).then(() => {
        ...
        saveDate() {
            this.$refs.dialog.$refs.form.validate(valid => {
                if (valid) {
                    ...
                    if (!obj.nextTime) obj.nextTime = '';
                    else obj.nextTime = moment(obj.nextTime).format('YYYY-MM-DD HH:mm:ss');
                    if (obj.cycleNum) obj.cycleNum = Number(obj.cycleNum);
                    obj.equipNum = Number(obj.equipNum);
                    obj.pointCheckRate = Number(obj.pointCheckRate);
                    if (this.dialogType === 'new') {
                        delete obj.pointCheckScheduleNo;
                        this.$http.post(`/devicemanager/pointcheckschedule/save`, obj).then(({ code, data }) => {
                            this.$message.success('新增成功');
                            this.dialogForm.forEach(v => {
                                if (v.prop === 'exe') v.value = data[exeTypeType[data['exeType']]];
                            ...
                            this.palnStatus = data.scheduleStatus;
                            this.parentRow = data;
                            // this.getSearchData();
                        }).catch(() => {
                            this.btnsForm[0].loading = false;
                        ...
                        if (this.palnStatus !== '0') {
                            ...
                            this.$http.put(`/devicemanager/pointcheckschedule/updateById`, obj).then(() => {
        ...
        SelectChange(selection) {
            ...
            this.filterData = this.selection.every(v => v.scheduleStatus === '0');
            this.filterData1 = this.selection.some(v => v.scheduleStatus === '1' || v.scheduleStatus === '3');
            this.filterData2 = this.selection.some(v => v.scheduleStatus !== '1');
        },
        // 下表
        getDataSon() {
            if (Object.keys(this.parentRow).length !== 0) {
                ...
                params.conditionList.push({
                    fieldName: 'pointCheckScheduleNo',
                    operation: 'EQ',
                    value: this.parentRow['pointCheckScheduleNo'],
                });
                this.$http.get(`/devicemanager/pointcheckschedulede/page`, {
                    params
                }).then(({ code, data }) => {
                    this.undertableData = data.records;
                    this.undertableData.forEach(v => {
                        v.pointCheckRate = this.parentRow.pointCheckRate;
                    });
        ...
        submitDataSon() {
            ...
            obj.pointCheckScheduleNo = this.dialogForm[0].value;
            obj.pointCheckPlanNo = this.CrtNo;
            this.undertableLoading = true;
            this.$http.post(`/devicemanager/pointcheckschedulede/save`, obj).then(res => {
        ...
        delRowSon() {
            ...
            this.$confirm('是否确认删除?', '提示', {
                ...
                this.submitLoading = true;
                this.$http.delete('/devicemanager/pointcheckschedulede/removeByIds', {
                    data: ids,
                }).then(() => {
                    this.submitLoading = false;
                    this.undercurrentPage = 1;
        ...
        exportAll() {
            ...
            this.pointCheckScheduleNo = this.parentRow['pointCheckScheduleNo'];
        },
        openEquip() {
        ...
        getEquip() {
            ...
            this.$http.get(`/devicemanager/pointcheckschedule/getDeviceEquipmentAccountPage/${this.parentRow['pointCheckScheduleNo']}`, {
                params
            }).then(res => {
        ...
        getPatrol() {
            ...
            const params = {
                ...
                conditionList: [{
                    fieldName: 'equipNum',
                    operation: 'EQ',
                    value: this.dialogFormSon[0].value,
                    joinType: 'AND'
                }, {
            ...
            this.$http.get(`/devicemanager/pointcheckplan/getPageByEquipNum`, {
                params
            }).then(res => {
        ...
        submitFormPatrol() {
            ...
            this.dialogFormSon[4].value = item.pointCheckPlanName;
            this.dialogVisiblePatrol = false;
            this.CrtNo = item.pointCheckPlanNo;
        },
